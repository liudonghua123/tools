...
export class WispNetworkBridge {
    ...
    #wake = () => true;
    #closed = false;
    constructor(url, dnsUrl = "https://cloudflare-dns.com/dns-query", requestUrl) {
    ...
    async connectTcp(_localText, peerText) {
        if (this.#closed)
            throw new Error("WISP network bridge is closed");
        const connection = await this.#getConnection();
        const peer = parseAddress(peerText);
        const destination = this.#resolvedHosts.get(stripIpv6Brackets(peer.host)) ?? peer.host;
    ...
    socketSetNoDelay(id, _enabled) {
        this.#requireSocket(id);
    }
    socketSetKeepAlive(id, _enabled) {
        this.#requireSocket(id);
    }
    socketRefresh(id) {
        queueMicrotask(() => {
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.onopen = () => {
                if (this.#closed || this.#openingConnection !== connection) {
                    rejectHandshake(new Error(this.#closed
            ...
            connection.onclose = () => {
                if (!settled) {
                    rejectHandshake(new Error(`WISP connection to ${endpoint.href} closed before its handshake completed`));
                    return;
                }
    ...
    #requireSocket(id) {
        const state = this.#sockets.get(id);
        if (!state)
            throw new Error(`unknown WISP socket ${id}`);
        return state;
    }
    #closeSockets() {
    ...
    #rememberResolvedHost(host, addresses) {
        for (const address of addresses) {
            this.#resolvedHosts.set(stripIpv6Brackets(address), host);
        }
    }
}
async function resolveIpAddresses(dnsEndpoint, hostname) {
...
async function resolveDnsType(dnsEndpoint, hostname, type) {
    ...
    return (body.Answer ?? []).filter((answer) => answer.type === type &&
        typeof answer.data === "string" &&
        (type === 1 ? isIpv4(answer.data) : isIpv6(answer.data))).map((answer) => ({
        address: answer.data,
        ttlSeconds: typeof answer.TTL === "number" && Number.isFinite(answer.TTL)
            ? Math.max(1, answer.TTL)
            : 60,
    }));
}
export function installWispNetworkGlobals() {
    ...
    scope.__wasmerHandleNetworkRpc = (value) => {
...
async function respondToNetworkRequest(request) {
    ...
    try {
        const bridge = bridgeFor(request.bridgeId);
...
function writeText(destination, value, truncate) {
...
function bridgeFor(id) {
    ...
    if (!bridge)
        throw new Error(`unknown or closed WISP network bridge ${id}`);
    return bridge;
}
...
function validateWispUrl(value) {
...
//# sourceMappingURL=wisp-network.js.map