...
const loadRustRuntime = async () => {
  ...
  loadingProgress.value = t('tools.code-playground.rust.loading', 'Loading Rune runtime...')
  
  try {
    const baseUrl = import.meta.env.BASE_URL || '/';
    const rustBase = `${baseUrl.endsWith('/') ? baseUrl : baseUrl + '/'}rust-wasm/`;
    ...
    await new Promise((resolve, reject) => {
      ...
      script.src = `${rustBase}rune.js`;
      script.onload = resolve;
      script.onerror = () => reject(new Error('Failed to load rune.js'));
      document.head.appendChild(script);
    });

    if (!window.rune) {
      throw new Error('Rune object not found after loading rune.js');
    }
    
    loadingProgress.value = t('tools.code-playground.rust.loading', 'Initializing Rune module...')
    await window.rune.init({ serverPath: rustBase });
    ...
    output.value.push({ 
      type: 'error', 
      message: `Failed to load Rust runtime: ${e.message}` 
    })
    console.error(e)
  } finally {
...
const runRust = async () => {
  if (!isReady.value) {
    await loadRustRuntime();
  }
  
  if (!window.rune || !window.rune.module) {
    output.value.push({ type: 'error', message: 'Rune interpreter not ready' });
    return;
  }
  ...
  try {
    ...
    if (result.diagnostics_output) {
        output.value.push({ type: 'error', message: result.diagnostics_output });
    }

    if (result.output) {
      output.value.push({ type: 'log', message: result.output })
    }

    if (result.result !== undefined && result.result !== null && result.result !== '()') {
        output.value.push({ type: 'log', message: `Result: ${result.result}` });
    }

    if (result.error) {
        output.value.push({ type: 'error', message: result.error });
    }
    ...
    <div class="w-full md:w-1/2 h-1/2 md:h-full flex flex-col bg-slate-900">
      ...
      <div v-else class="flex-1 overflow-y-auto p-4 font-mono text-sm space-y-1">
        ...
        <div v-if="output.length === 0 && !isLoading" class="text-slate-500 italic">
          Click "Run" to execute Rust (Rune) code...
        </div>
      </div>
    </div>
  </div>
